...
class LongTextRequest(BaseModel):
    ...
    input: str = Field(
        ...,
        min_length=3001,
        description="Text to convert to speech (must be > 3000 characters)",
    )
    voice: Optional[str] = Field(
        None, description="Voice name from library or OpenAI voice name"
    )
    response_format: Optional[str] = Field(
        "mp3", description="Audio format (mp3 or wav)"
    )
    exaggeration: Optional[float] = Field(
        None, ge=0.25, le=2.0, description="Emotion intensity"
    )
    cfg_weight: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Pace control"
    )
    temperature: Optional[float] = Field(
        None, ge=0.05, le=5.0, description="Sampling temperature"
    )
    session_id: Optional[str] = Field(
        None, description="Frontend session ID for tracking"
    )
    ...
    def validate_input_length(cls, v):
        if (
            len(v) > 100000
        ):  # Will be validated against Config.LONG_TEXT_MAX_LENGTH at runtime
...
class LongTextJobMetadata(BaseModel):
    ...
    text_hash: str = Field(
        ..., description="SHA256 hash of input text for deduplication"
    )
    total_chunks: int = Field(..., ge=1, description="Total number of chunks")
    completed_chunks: int = Field(
        default=0, ge=0, description="Number of completed chunks"
    )
    failed_chunks: List[int] = Field(
        default_factory=list, description="Indices of failed chunks"
    )
    current_chunk: Optional[int] = Field(
        None, description="Currently processing chunk index"
    )
    voice: Optional[str] = Field(None, description="Voice used for generation")
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="TTS parameters used"
    )
    processing_started_at: Optional[datetime] = None
    processing_paused_at: Optional[datetime] = None
    processing_completed_at: Optional[datetime] = None
    total_processing_time_ms: int = Field(
        default=0, ge=0, description="Total processing time"
    )
    output_format: str = Field(default="mp3", description="Output audio format")
    output_path: Optional[str] = Field(
        None, description="Path to final concatenated audio"
    )
    output_size_bytes: Optional[int] = Field(
        None, ge=0, description="Final audio file size"
    )
    output_duration_seconds: Optional[float] = Field(
        None, ge=0, description="Final audio duration"
    )
    error: Optional[str] = None
    ...
    completion_timestamp: Optional[datetime] = Field(
        None, description="When job fully completed successfully"
    )
    audio_file_path: Optional[str] = Field(
        None, description="Persistent path to final audio file for history"
    )
    audio_file_size: Optional[int] = Field(
        None, ge=0, description="Size of final audio file in bytes"
    )
    total_duration_seconds: Optional[float] = Field(
        None, ge=0, description="Total audio duration in seconds"
    )
    retry_count: int = Field(
        default=0, ge=0, description="Number of times job has been retried"
    )
    original_job_id: Optional[str] = Field(
        None, description="Original job ID if this is a retry"
    )
    is_archived: bool = Field(
        default=False, description="Whether job is archived in history"
    )
    last_accessed: Optional[datetime] = Field(
        None, description="When user last interacted with this job"
    )
    display_name: Optional[str] = Field(
        None, description="User-friendly name for the job"
    )
    tags: List[str] = Field(
...
class LongTextJobResponse(BaseModel):
    ...
    download_url: Optional[str] = Field(
        None, description="URL to download completed audio"
    )
...
class LongTextJobList(BaseModel):
    ...
    active_jobs: int = Field(
        ..., ge=0, description="Jobs that are pending or processing"
    )
...
class LongTextSSEEvent(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Event type: progress, status_change, error, completed"
    )
...
class LongTextJobRetryRequest(BaseModel):
    ...
    preserve_chunks: bool = Field(
        default=True, description="Whether to preserve successful chunks"
    )
    new_parameters: Optional[Dict[str, Any]] = Field(
        None, description="Updated TTS parameters"
    )
...
class LongTextJobDetails(BaseModel):
    ...
    error_log: List[str] = Field(
        default_factory=list, description="Detailed error history"
    )
...
class BulkJobActionResponse(BaseModel):
    ...
    failed_jobs: List[str] = Field(
        default_factory=list, description="Job IDs that failed to process"
    )
    message: str
