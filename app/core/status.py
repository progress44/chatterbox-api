...
class TTSStatusManager:
    ...
    def update_status(
        ...
        with self._lock:
            if (
                ...
                or self._current_request.request_id != request_id
            ):
                return
    ...
    def get_current_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            request_dict["progress"]["progress_percentage"] = (
                self._current_request.progress.progress_percentage
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            completed_requests = [
                r for r in self._request_history if r.status == TTSStatus.COMPLETED
            ]
            error_requests = [
                r for r in self._request_history if r.status == TTSStatus.ERROR
            ]

            if completed_requests:
                avg_duration = sum(
                    r.duration_seconds for r in completed_requests
                ) / len(completed_requests)
                avg_text_length = sum(r.text_length for r in completed_requests) / len(
                    completed_requests
                )
            else:
                avg_duration = 0
            ...
            return {
                ...
                "success_rate": (
                    len(completed_requests)
                    / max(1, len(completed_requests) + len(error_requests))
                )
                * 100,
