...
def concatenate_audio_files(
    ...
    logger.info(
        f"Concatenating {len(audio_files)} audio files with {silence_duration_ms}ms silence padding"
    )
    ...
    try:
        ...
        for i, audio_file in enumerate(audio_files):
            ...
            try:
                ...
                logger.debug(
                    f"Loaded audio segment {i + 1}/{len(audio_files)}: "
                    f"{len(audio)} ms, {audio.frame_rate} Hz, {audio.channels} channels"
                )

            except Exception as e:
                raise AudioConcatenationError(
                    f"Failed to load audio file {audio_file}: {e}"
                )
        ...
        silence = AudioSegment.silent(
            duration=silence_duration_ms, frame_rate=segments[0].frame_rate
        )
        ...
        result.export(str(output_path), format=output_format, **export_params)
        ...
        logger.info(
            f"Audio concatenation successful: {duration_seconds:.1f}s, "
...
def _standardize_audio_properties(segments: List[AudioSegment]) -> List[AudioSegment]:
    ...
    for i, segment in enumerate(segments):
        ...
        if segment.frame_rate != target_frame_rate:
            standardized_segment = standardized_segment.set_frame_rate(
                target_frame_rate
            )
            logger.debug(
                f"Converted segment {i} from {segment.frame_rate} Hz to {target_frame_rate} Hz"
            )
        ...
        if segment.channels != target_channels:
            ...
            logger.debug(
                f"Converted segment {i} from {segment.channels} to {target_channels} channels"
            )
