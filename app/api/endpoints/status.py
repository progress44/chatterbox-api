...
async def get_processing_status(
    ...
    history_limit: int = Query(
        5, description="Number of history records to return", ge=1, le=20
    ),
) -> Dict[str, Any]:
...
async def get_request_history(
    limit: int = Query(
        10, description="Number of history records to return", ge=1, le=50
    ),
) -> Dict[str, Any]:
    """Get TTS request history"""
    history = get_tts_history(limit)

    return {"request_history": history, "total_records": len(history), "limit": limit}
...
async def clear_request_history(
    ...
    try:
        clear_tts_history()
        return {"success": True, "message": "TTS request history cleared successfully"}
    except Exception as e:
