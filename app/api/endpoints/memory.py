...
def get_memory_alerts(memory_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # CPU memory alert
    if (
        memory_info.get("cpu_memory_percent", 0)
        > MEMORY_ALERT_THRESHOLDS["cpu_memory_percent"]
    ):
        alerts.append(
            {
    ...
    return {"has_alerts": len(alerts) > 0, "alert_count": len(alerts), "alerts": alerts}
...
async def reset_memory_tracking(
    confirm: bool = Query(False, description="Confirm the reset operation"),
):
    ...
    try:
        ...
        print(
            f"🔄 Memory tracking reset: counter {old_counter} → 0, collected {collected} objects"
        )
        ...
        return {"status": "error", "error": str(e), "memory_info": get_memory_info()}
...
async def update_memory_config(
    cpu_memory_percent: Optional[float] = Query(
        None, description="CPU memory alert threshold (percentage)", ge=50, le=95
    ),
    gpu_memory_mb: Optional[float] = Query(
        None, description="GPU memory alert threshold (MB)", ge=1024, le=32768
    ),
):
