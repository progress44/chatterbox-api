# Chatterbox TTS V2 for Olares

This package deploys the published image:

- `ghcr.io/progress44/rpi-system-chatterbox-tts-api:v2-blackwell`
...
attempt to pull the configured V2 image tag on start instead of reusing a
cached node image.
...
- `http://chatterboxtts-v2-svc:4123`
...
```bash
curl -X POST http://chatterboxtts-v2-svc:4123/tts \
...
```bash
curl -X POST http://chatterboxtts-v2-svc:4123/v1/audio/speech \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://chatterboxtts-v2-svc:4123/v1/audio/speech/upload \
...
- Runtime tuning is controlled through V2 config env vars such as `DEVICE`,
  `MAX_TOTAL_LENGTH`, `EXAGGERATION`, `CFG_WEIGHT`, and `TEMPERATURE`.
...
```bash
kubectl logs -n chatterboxtts-v2-progress44 deploy/chatterboxtts-v2 -f
```
